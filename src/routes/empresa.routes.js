...
router.route("/").get(async (req, res) => {
    try {
        const resultado = await query(
            `SELECT idEmpresa, nomeEmpresa, emailEmpresaContato, cnpjEmpresa, usuario FROM tbEmpresa JOIN tbUsuario ON tbUsuario.idUsuario = tbEmpresa.idUsuario WHERE statusEntidade = 'ativo'`
        );
        res.json(resultado);
    } catch (erro) {
        res.status(500).json({
...
router
    ...
    .post(upload.none(), async (req, res) => {
        ...
        try {
            ...
            const resultado = await query(
                `SELECT idEmpresa FROM tbEmpresa JOIN tbUsuario ON tbEmpresa.idUsuario = tbUsuario.idUsuario WHERE tbUsuario.usuario = '${usuario}'`
            );
    ...
    .put(upload.none(), async (req, res) => {
        ...
        try {
            ...
            let logradouroAtualizado =
                enderecoAtual[0].logradouroEnderecoEmpresa;
            if (validaLogradouro(logradouro)) {
...
router.route("/endereco/:usuario/:id").delete(async (req, res) => {
    ...
    try {
        ...
        const consulta = query(
            `DELETE FROM tbEnderecoEmpresa WHERE idEmpresa = ${empresa.idEmpresa} AND idEnderecoEmpresa = ${id}`
        );
    ...
    .put(upload.none(), async (req, res) => {
        ...
        try {
            ...
            let visibilidadeAtualizada =
                telefoneAtual[0].visibilidadeTelefoneEmpresa;
...
router.route("/telefone/:usuario/:id").delete(async (req, res) => {
    ...
    try {
        ...
        const consulta = query(
            `DELETE FROM tbFoneEmpresa WHERE idEmpresa = ${empresa.idEmpresa} AND idFoneEmpresa = ${id}`
        );
